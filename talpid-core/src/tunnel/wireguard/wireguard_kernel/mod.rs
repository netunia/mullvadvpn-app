...
impl Tunnel for KernelTunnel {
    fn get_interface_name(&self) -> String {
        let mut netlink = self.netlink_connections.clone();
        let result = self.tokio_handle.block_on(async move {
            let device = netlink.get_by_index(self.interface_index).await?;
    ...
    fn stop(self: Box<Self>) -> std::result::Result<(), TunnelError> {
        let mut netlink_connections = self.netlink_connections.clone();
    ...
    fn get_tunnel_stats(&self) -> std::result::Result<Stats, TunnelError> {
        ...
        let interface_index = self.interface_index;
        let result = self.tokio_handle.block_on(async move {
            let device = netlink.get_by_index(interface_index).await.map_err(|err| {
...
#[derive(Clone, Debug)]
pub struct Handle {
    wg_handle: ConnectionHandle<DeviceMessage>,
...
impl Handle {
    ...
    pub async fn create_device(&mut self, name: String, mtu: u32) -> Result<u32, Error> {
        ...
        let new_device = self.get_by_name(name).await?;
